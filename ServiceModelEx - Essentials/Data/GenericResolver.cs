...
namespace ServiceModelEx
{
   public class GenericResolver : DataContractResolver
   {
      ...
      static bool IsDomainAssembly(Assembly assembly)
      {
         //TEST ONLY...
         //bool isDomainAssembly = IsSystemAssembly(assembly.GetName()) == false && assembly.IsDynamic == false;
         bool isDomainAssembly = IsSystemAssembly(assembly.GetName()) == false;
      ...
      static Assembly[] GetCustomReferencedAssemblies(Assembly assembly)
      {
         ...
         foreach(AssemblyName assemblyName in referencedAssemblies.Where(name=>IsSystemAssembly(name) == false))
         {
            try
            {
               Assembly referencededAssembly = Assembly.Load(assemblyName);
               if(referencededAssembly.IsDynamic == false)
               {
                  assemblies.Add(referencededAssembly);
               }
            }
